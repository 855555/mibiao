...
<div style="display: flex; align-items: center;">
  <div class="layui-inline">
    <a href="index.php">
      <button type="button" class="layui-btn" style="height: 38px;">显示全部</button>
    </a>
  </div>
  ...
  <div class="layui-inline">
   ...
   <tbody>
<?php
if (isset($_POST['yuming-search'])) {
    $yuming = $_POST['yuming'];
    if ($yuming == '') {
        echo '<script>alert("搜索框内容不能为空");window.location.href="index.php";</script>';
        exit;
    }

    $sql = "SELECT * FROM mi WHERE yuming = '$yuming' ORDER BY id";
} elseif (isset($_POST['zhuangtai-search'])) {
    $zhuangtai = $_POST['zhuangtai'];
    if ($zhuangtai == '') {
        echo '<script>alert("请选择状态");window.location.href="index.php";</script>';
        exit;
    }

    $sql = "SELECT * FROM mi WHERE zhuangtai = '$zhuangtai' ORDER BY id";
}
...
if (mysqli_num_rows($rs) < 1) {
    echo '<script>alert("未检索到相关结果");window.location.href="index.php";</script>';
    exit;
}
...
while ($rows = mysqli_fetch_assoc($rs)) {
    ...
    echo '<tr>';
    echo '<td>' . $rows['id'] . '</td>';
    echo '<td>' . $rows['yuming'] . '</td>';
    echo '<td>' . $rows['zhuangtai'] . '</td>';
    echo '</tr>';
}
?>
</tbody>
    ...
    window.location.href = 'edit.php?id=' + tr.id;
        ...
        table.render({
          ...
          cols: [[
            ...
            { field: 'yuming', sort: true, title: '域名' },
